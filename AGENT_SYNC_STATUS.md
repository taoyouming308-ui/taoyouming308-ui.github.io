...
## 上线整改：B06 工资/业绩原表候选请求竞态（2026-09-24，v540 本地待发布）

- App version: v540。工资表和发型师业绩表的原表单元格异步读取现绑定当前请求序号、门店、月份、视图及选中原表；旧请求即使在新结果之后成功或失败也不会覆盖候选，清空来源同样使旧请求失效。
- 新增合成浏览器回归并纳入统一财务测试清单，覆盖工资旧失败/清空来源以及业绩旧成功迟到。隔离浏览器定向测试及统一财务测试 64/64 通过；完整 pre-push 发布钩子尚未运行，也未提交、推送或发布。
- 共享版本校验要求自由手艺人 App、财务页与前台页使用同一发布号；同步三个入口的版本标记/缓存版本，不改变前台功能。
- 未改工资/业绩计算、数据库、财务数据、凭证、权限或财务独立登录入口。下一步需完整门禁与发布核验。
