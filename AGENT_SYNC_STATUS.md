...
- 多关联校正前的一次性 PostgreSQL 15 已实际执行迁移并通过财务允许、普通股东/跨店拒绝、重复阻断、审核状态、不可变审核历史、单报表关联、审计快照、OCR候选不转正式、RLS 和浏览器无写 RPC 权限；旧测试容器已停止并删除。随后按 V2.0“同一凭证可关联多个业务记录”改为多报表关联，本轮重新下载 PostgreSQL 15 镜像持续无进度而终止，因此当前最终迁移的精确数据库重放仍待补，禁止据此直接生产迁移。完整 pre-push 已通过；浏览器已验证两张报表同时勾选、财务上传/审核、股东只读、原图与候选并排和零控制台错误。
