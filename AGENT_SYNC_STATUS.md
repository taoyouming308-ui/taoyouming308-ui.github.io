...
- 新增 `mgj-booking-sync` Edge Function v1（生产 ACTIVE），只接受本机 Ed25519 签名、两家白名单门店和北京时间当天起 8 天的预约；每个门店/日期先写入再按该门店/日期清理源头已不存在的记录。签名私钥只在本机 Hermes 运行目录（0600），没有进入仓库或前端。
...
- 本地预约/保活/顾客档案/健康审计 Python 回归 47/47、员工预约合并、版本同步、发布完整性、App smoke、预约 UI、护理出库和交接门禁均通过；运行脚本与仓库 SHA-256 一致。完整 pre-push 门禁和 GitHub CI 结果待记录。未来自动轮次及已登录员工 App 界面仍需观察；本次未改顾客档案同步、护理出入库或财务流程。
