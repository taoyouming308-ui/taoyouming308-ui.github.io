...
- 三项生产迁移已应用，`operations-api` version 61 为 ACTIVE，保持 `verify_jwt=false` 与函数内部会话鉴权；匿名 overview 返回 403 `AUTH_SESSION_INVALID`。完整 pre-push 门禁通过。线上以财务账号实测「向里造型 / 2026-09」成功建立电子月报 v1，复用本店原版模板并登记 328 个数字/公式单元格；金额可进入「金额与原始凭证」工作台，验收期间未改动任何金额。
