...
- 原图支持左右 90 度旋转、恢复方向，文件字节不修改。当前日报 JPG/PNG 上传后请求既有 Kimi 配置，空白格填入候选并高亮；已有金额保留，日期/门店冲突拒绝填入，结果先写审计，财务保存并最终确认后才进入正式账。
- 汇总按同公司同门店同月已确认日报唯一日期及 grand_total 计算，重复日期或无效总计拒绝汇总；月报保留原数、展示已确认天数与差额，可进入具体日报。未确认、未录齐月份不视为完整；无已确认日报时显示 0 和提示，不沿用旧收入冒充日报汇总。
- 新增 daily_rollup_adjustment_snapshot 迁移，月报调整校验已确认日报版本，日报状态变动使用同一月锁。未应用生产迁移、未部署，未改写生产账或调用真实图片识别；模型密钥名称已核实存在，实际识别精度与服务可用性待真实图验收。
