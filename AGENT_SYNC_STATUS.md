...
## v506 电子日报候选核对、保存反馈与月报收入去重（2026-09-20，发布中）
...
- 隔离 worktree 开发，不修改生产财务数据或启动已停止的日报识别执行器。完整 pre-push 门禁、独立数据库回归及 v506 候选／月报去重回归已通过。生产 `operations-api` v80 已部署为 ACTIVE，保持 `verify_jwt=false` 与函数内部会话鉴权；匿名探针返回 403 `AUTH_SESSION_INVALID`。GitHub Pages 页面尚未发布。
