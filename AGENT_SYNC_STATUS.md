...
## 护理出库回写容错（2026-07-21，不发版）

- 真实批次 `FC-117776444336241` 在美管加已生成并审核单号 `CPKY20260721001`，4 项明细、员工和克数回查一致；故障发生在最终 Supabase 完成状态回写，不是账号或门店错误。
- 对该批次 4 条确定性队列 ID 做了精确修复，从 `failed` 更新为 `completed` 并保存已审核单号；没有再次调用美管加创建或审核接口。
- `care_outbound_worker.py` 对 Supabase 短时网络错误、408、429 和 5xx 增加最多 3 次退避重试。
- 已审核单据若最终回写仍失败，队列只进入 `needs_review` 并带“禁止重复出库”提示，不再降级成可误解的普通 `failed`。
- 新增瞬时网络重试与已审核回写失败保护测试；App version 保持 v390。
