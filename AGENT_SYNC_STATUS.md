...
## v500 月报名与明细名称可编辑（2026-09-13，发布中）
...
- 本地页面语法、月报金额回归、新名称浏览器回归及隔离 PostgreSQL 权限/锁账/审计/不可篡改测试已通过。生产迁移 `zysyr_monthly_editable_names` 已应用；新表启用并强制 RLS，浏览器角色不能执行写入 RPC；`operations-api` version 68 已 ACTIVE，保持 `verify_jwt=false` 与函数内部会话鉴权，匿名请求返回 403 `AUTH_SESSION_INVALID`，在线源码已核验名称识别、保存路由与修订表读取。正在发布 GitHub Pages。
