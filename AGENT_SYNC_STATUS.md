...
## v501 月报备用金直达与导航精简（2026-09-14，待发布）
...
- 备用金汇总格无需单独凭证，凭证继续关联到逐笔备用金记录，追溯链保持为“月报备用金 → 备用金明细 → 单笔原始凭证”。本地浏览器回归已验证汇总直达、门店/月不变、右侧单项不误跳转和三个导航入口隐藏。`operations-api` version 69 已 ACTIVE，保持 `verify_jwt=false` 与函数内部会话鉴权，在线源码含新分类且匿名请求返回 403 `AUTH_SESSION_INVALID`；正在发布 GitHub Pages。
