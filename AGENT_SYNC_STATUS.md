...
## v438 4月人工日报测试导入保护（2026-08-26，生产发布候选）

- 4月26张日报原图已人工核对日期映射，覆盖除四个星期一（6、13、20、27）外的全部营业日；生产当前没有4月报表。
- `voucher_upload` 新增仅内部导入工具使用的 `skip_ocr=true`，日报原图上传后不唤醒 OCR worker，随后人工审核会取消排队任务；原图、SHA-256、审核记录和后续电子表追溯均保留。
- 新增无密钥导入工具 `scripts/import-zysyr-april-test.mjs`；财务密码只经隐藏环境变量传入。待完成回归、函数部署和静态页发布后，再执行26张生产测试原图/草稿导入。
- App version: v438
