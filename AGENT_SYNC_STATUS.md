...
- 美管加会话 keepalive 验证 healthy，但 `memberDetail!detail.action` 仍返回业务 code 403；现金页回退可更新档案但不能产生新的权威项目明细。下一步必须从已登录美管加页面抓取真实 Network 请求，禁止猜测接口或字段。
- GitHub Pages 已验证 `version.txt=382`，后台包含美管加漏单对账模块。
- App version: v382
