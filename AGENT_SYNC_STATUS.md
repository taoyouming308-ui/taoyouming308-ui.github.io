...
## v496 日报原图方向保存（2026-09-13，待发布）

- App version: v496。基于已发布 v495，为财务端日报每一张 JPG/PNG 原图增加独立的「保存方向」操作；向左转、向右转或自动摆正后保存，退出、刷新及重新打开仍保持该图片方向。
- 原始图片文件保持不可变；显示方向以追加版本保存，记录门店、日报、附件、修改人、时间、前后角度和原因。只允许具有 `daily_report.write` 的账号保存，股东仍为只读。
- 本版本新增数据库迁移并修改 `operations-api`；发布前需迁移生产数据库、部署函数并发布静态页。隔离 PostgreSQL 已验证权限、审计、追加历史和原图不变，桌面/手机横竖屏浏览器已验证保存后重新打开仍居中且保持方向。
