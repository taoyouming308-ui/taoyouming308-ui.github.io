...
- 新增外部渠道回执追加历史、按门店授权读取 RPC、登记/通过/拒绝 API 与工作台表单；登记人与复核人必须分离，拒绝后可追加新版本。退款执行器在任一外部支付缺少同金额、最新版本 `verified` 回执时整笔回滚，不产生支付反向流水、会员账、库存或订单退款金额变化。
- 临时 PostgreSQL/Chrome 覆盖登记、同员工自审拒绝、另一员工拒绝、修正后新版本登记、他人复核通过、刷新回显及零资金/库存/订单/业绩副作用；API 输入校验、请求恢复和模型测试通过。
- 不接支付渠道，不证明渠道退款真实到账；退款执行 UI 仍关闭。正式 Auth/Edge/Advisor 与线下凭证流程验收仍待完成；不得应用远程迁移、部署或合并到三个现有板块。
- 变更范围：`supabase/migrations/20260924052952_salon_refund_channel_receipts.sql`、`20260924054029_salon_refund_channel_receipt_api.sql`、API/工作台、合成回归与文档。
...
- 新增 Salon 退款申请人撤回能力：仅原申请人可撤回本店 `submitted` 状态申请；完整快照与行锁/CAS 防止与审批竞态，审批/拒绝/执行后不可撤。撤回人、时间、原因独立留痕并追加审计，`cancelled` 释放在途退款额度。
