...
## v489 日报完整候选与可恢复进度（2026-09-12，生产后端已部署，静态页待发布）

- App version: v489。基于 GitHub main v488，只修复经营驾驶舱日报图片候选识别与整月任务进度；生产数据库与函数已部署，静态页待推送 GitHub main。
...
- 待发布步骤：复核完整门禁后推送静态 v489，等待 Validate 与 Pages 成功，再执行 CDN 无缓存核验。生产识别仍只生成待财务核对候选，不自动确认或入账。
