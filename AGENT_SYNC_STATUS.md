...
- App version: v482 基线；此次只增量开发，未发布新版本。
- 新增 `20260924040742_salon_refund_request_withdrawal.sql`：仅原申请人可撤回本店 `submitted` 申请，需完整快照；与审批争用退款行锁，审批/拒绝/执行后不可撤。单独记录撤回人、时间、原因及追加审计，`cancelled` 释放在途退款额度。
- 本机工作台新增“撤回本人待审批申请”按钮；多层门店权限、完整参数指纹、同键幂等、页面回读与刷新后 request lookup 恢复。操作不会写支付/会员/库存/业绩/订单退款额。
- 专项 PG15/手机浏览器测试通过；Salon 全部 `.mjs` 测试通过；退款申请、部分退款、审批、现金收银既有 PG/浏览器回归通过；员工带请求号函数覆盖增至 48。详见 `docs/salon-refund-withdrawal.md`。
- 迁移仅独立分支本地测试，未应用默认 Supabase、未部署、未推 main，未整合旧三 App。商品验收返库、真实退款凭证/执行保护、会员/组合支付、正式 Auth/Edge/Advisor 仍待完成。

## Salon 独立分支同步 v482 基线（2026-09-08）

- App version: v482。仅同步上游 b485f3a 已存在的主线提交到独立分支，保留双方交接记录；本批 Salon 功能提交 9f37b15 不发布。
- Salon 最终 1280/390 多次申请、剩余额度禁选、已执行/在途同时展示、旧审批详情清除及回读恢复专项通过；既有全退、审批、回读异常恢复均通过，手机截图已复查。此次合并未改 Salon 功能文件。
- 本任务不推 main、不部署 operations 或 Salon 的函数/迁移、不写真实业务数据，不合并原三个 App。上游自身发布状态以其记录为准，不由本任务执行。
...
- 页面接模式选择/逐项输入/冻结分配预览/提交回读/他人审批；改输入必须重做预览。同键/刷新恢复逐项核对原回执，不只看合计，清单只存操作/请求号。不执行资金或返库。
- 全量 Salon JS/MJS、新 PG/1280/390 专项及既有全退/审批/回读异常恢复通过；含多行分配、全退与部分退竞争、非法精度/越界/重复行、撤权/跨店/本人禁审、审计回滚、断网和修改预览。申请前后支付/订单/库存/会员/业绩无变化。
- Supabase/Postgres 技能用于权限与锁序；默认本地实例不可用，Advisor 未完成，未切生产。已退过订单、在途占用与剩余额度、多次申请、实际退款/返库核实、非现金支付、正式 Auth/Edge 与原三 App 整合仍待完成。
- 今日备份已确认，仅 feature/meiguanjia-parity-v1；不推 main、不部署迁移或函数。细节见 docs/salon-partial-cash-refund.md。

## Salon 现金全额退款申请（2026-09-08）

- App version: v481。CLI 新建 20260908060101_salon_cash_refund_request.sql：仅单笔现金、未退过、正价完整明细且无活动申请；原单/支付锁后完整快照比较，完整员工指纹、防重、审计回滚，service_role-only。员工请求函数覆盖 46（不是模块数）。
- 页面接原单编号/原现金支付/项目分配核对 → 原因 → 人工确认创建 → 回读详情 → 他人审批；仅申请，不退款/退会员/返库。错回执、旧快照不放行，丢包原键及刷新只读恢复，清单仅操作/请求号。
- 新模型/API、隔离 PG、1280/390 Chrome 专项及全量 Salon JS/MJS 通过；现金收款、退款审批、刷新恢复回归通过；支付/订单/库存/会员/业绩快照未变。手机申请详情截图已检查。
- Supabase/Postgres 技能用于最小权限与事务锁序；默认本地实例未启动，Advisor 未完成，未切生产。部分退款、实际执行/返库核实、会员/组合支付、正式 Auth/Edge 与原三个 App 整合仍未完成。
- 今日备份已确认，仅 feature/meiguanjia-parity-v1 开发备份；不推 main、不部署迁移/函数、不读真实资金。详情 docs/salon-cash-refund-request.md。
