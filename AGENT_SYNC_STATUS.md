...
## 上线整改：B08 历史凭证预览按需加载（2026-09-24，本地待完整门禁）
...
- 统一财务测试清单 63/63、凭证浏览器桌面/手机竖屏/横屏、版本/发布完整性/同步状态检查和 `git diff --check` 全部通过。代码尚未推送；整包 Edge 部署和 GitHub Pages 发布仍待完成。未连接真实财务账号或生产文件，不能声称线上已生效。
