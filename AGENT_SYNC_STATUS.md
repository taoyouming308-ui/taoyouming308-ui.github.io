...
## v489 日报完整候选与可恢复进度（2026-09-12，本地待发布）

- App version: v489。基于 GitHub main v488，只修复经营驾驶舱日报图片候选识别与整月任务进度，尚未部署生产或推送 GitHub。
...
- 待发布步骤：复核完整门禁后，先迁移数据库，再部署 `operations-api` 与本机桥接，最后推送静态 v489 并进行匿名 403、任务恢复和财务核对回归。
