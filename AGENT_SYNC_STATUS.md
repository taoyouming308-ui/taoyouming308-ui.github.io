...
## v496 日报原图方向保存与新月编辑修复（2026-09-13，待发布）
...
- 本版本新增两项数据库迁移并修改 `operations-api`；发布前需迁移生产数据库、部署函数并发布静态页。隔离 PostgreSQL 已验证权限、审计、追加历史、原图不变及月报来源对象共享边界，桌面/手机横竖屏浏览器已验证保存后重新打开仍居中且保持方向。
