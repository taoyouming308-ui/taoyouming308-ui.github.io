...
- 生产迁移 `20260923105746_zysyr_daily_stylist_subtotal_validation` 已应用；只读核验 2 月 5 日校验通过，2 月 3、7 日新规则准确报告小计差异。`operations-api` v94 ACTIVE，继续 `verify_jwt=false` 与函数内部会话鉴权，匿名请求返回 403 `AUTH_SESSION_INVALID`。安全／性能顾问无本次对象新增错误，原有无关 RLS 提示未改。GitHub Pages 发布待完成；未代财务修改正式数据或自动入账。
