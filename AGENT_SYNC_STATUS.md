...
## v525 月报内嵌整月日报业绩（2026-09-23，待发布）
...
- 无数据库迁移。完整 `.githooks/pre-push` 门禁已通过，覆盖静态/API语法、月报审计、日报月历、月报载入、整月映射与合计、原月报缩放、名称编辑、月报照片上传、多月汇总关闭，以及 1280×900、390×844、844×390 真实浏览器回归。生产 `operations-api` 与 GitHub Pages 尚未发布，现网仍为 v524；发布时需重新同步 github/main，并同时部署 Edge Function 与静态页面。
