...
- 新增外部渠道退款回执追加历史与登记/复核分离约束；数据库退款执行器在任一现金/微信/支付宝/银行卡分配缺少同金额、最新版本的 `verified` 回执时整笔回滚，不产生支付反向流水、会员账、库存或订单退款金额变化。
- 通过单员工自审、待复核回执执行阻断、双人复核后合成执行等隔离 PG/浏览器用例；静态 SQL/API/审批模型回归通过。
- 本批不接支付渠道，不证明渠道退款真实到账；回执录入/复核工作台 API 和界面仍未接通，退款执行 UI 仍关闭。不得应用远程迁移、部署或合并到三个现有板块，待离线测试和用户上线决策。
- 变更范围：`supabase/migrations/20260924052952_salon_refund_channel_receipts.sql`、对应合成回归与 `docs/salon-refund-channel-receipts.md`。
