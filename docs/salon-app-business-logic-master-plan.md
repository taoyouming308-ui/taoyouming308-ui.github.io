...
以下是基于本地源码的开发待办，不是已修复结论。

| 编号 | 已发现问题或证据不足 | 必须完成的交付 |
|---|---|---|
| G01 | 页面仍分别读写 localStorage，没有统一正式业务入口 | 接入认证、门店上下文和 API；保留独立模拟模式；页面端到端测试 |
| G02 | 领域串联测试由测试脚本手动扣库存、记业绩、建服务 | 页面操作触发服务端事务，验证一次确认自动得到一致的结果 |
| G03 | 订单小票、库存的部分读取路径没有注入操作者做细粒度授权 | 全部资源读写统一权限检查，含本人范围及失效门店 |
| G04 | 顾客端多个操作在对象归属检查前可能返回共享幂等结果；部分状态请求只绑定对象 | 绑定操作者和完整载荷，授权先于缓存返回；跨账号重放测试 |
| G05 | 顾客账号绑定仅由员工输入账号标识；照片资产目前只是文本引用 | 双方确认绑定；私有资源所属关系、受控访问、上传回查 |
| G06 | 已确认顾客预约取消只有待处理状态；未形成改期闭环 | 门店批准/拒绝取消、释放档期、改期事务、顾客结果展示 |
...
| G12 | 已有 PG 回放不等于实际服务角色、Edge 运行时和浏览器联调验收 | 三层验证：受限数据库角色、接口鉴权、页面真实交互 |
