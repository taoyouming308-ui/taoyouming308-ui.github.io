...
|---|---|---|
| G01 | 原 13 模块仍分别读写 localStorage；统一客户端及会话保护已接本机工作台，建档/开单/商品明细已连通真实 SQL；Auth 仍为合成替身 | 确认专用测试项目及登录方式、接入正式 SDK 并逐模块迁移；补齐跨刷新恢复及完整页面端到端测试 |
...
| G06 | 已补取消复核、门店原子改期及顾客申请/门店复核；双端接鉴权门店时区，HTTP/PG、手机和丢包重试通过 | 仍需配置变更事务内保护、正式顾客登录、例外处理和通知；详见 salon-store-time.md、salon-customer-reschedule-requests.md、salon-booking-cancel-review.md 与 salon-booking-reschedule.md |
