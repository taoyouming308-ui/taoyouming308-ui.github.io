...
退款审批已接入：本店申请分页 → 读取原单/原支付分配 → 双人批准/拒绝 → 回读确认；快照变化拒绝旧审批，未知结果同键/只读恢复。默认角色无审批权限，专项测试使用独立合成双人夹具。批准不等于已退款；无退款执行按钮。见 `salon-refund-review.md`。

现金全退申请已接入：本店原单编号 → 原现金支付/全部项目核对 → 原因 → 创建申请 → 回读 → 他人审批。仅单笔现金、正价全单、从未退款且无活动申请；不接部分、会员、组合或零元赠送。支持原键及刷新恢复，不产生资金或库存变动。见 `salon-cash-refund-request.md`。
