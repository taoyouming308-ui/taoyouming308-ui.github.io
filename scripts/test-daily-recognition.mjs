...
const definitions=['cleanText','uuidIn','effectiveCellValue','confirmedDailySource','confirmedDailyRollupFromSource','confirmedDailyRollup'].map(name=>{const start=source.indexOf((['confirmedDailyRollup','confirmedDailySource'].includes(name)?'async ':'')+'function '+name+'(');return source.slice(start,source.indexOf('\n}',start)+2);}).join('\n');
const id='00000000-0000-4000-8000-000000000001';
let duplicate=false;
const context=vm.createContext({restRowsAll:async path=>{assert.match(path,/company_id=eq.company/);assert.match(path,/store_id=eq.store/);if(path.startsWith('zysyr_daily_sheet_drafts')){assert.match(path,/status=eq.confirmed/);return duplicate?[{id,report_date:'2026-04-01'},{id,report_date:'2026-04-01'}]:[{id,report_date:'2026-04-01',edit_revision:1,confirmed_at:'2026-09-20T00:00:00Z'}];}assert.match(path,/column_code=in\.\(grand_total,cash_flow/);return [{id:'cell',draft_id:id,section_code:'payment',column_code:'cash_flow',row_key:'payment',cell_role:'payment_cashflow',manual_override:true,corrected_numeric:123.45}];}});
