...
insert into public.salon_role_permissions(role_id,resource,action) values(1,'orders','refund_execute'),(1,'orders','refund_read'),(1,'inventory','write');
insert into public.salon_staff(organization_id,store_id,role_id,staff_no,display_name) values(1,1,1,'A01','甲店执行人'),(1,2,1,'B01','乙店执行人'),(1,1,1,'A02','甲店复核人');
...
do $$ declare first jsonb;retry jsonb;blocked boolean;begin
 ...
 blocked:=false;begin perform public.salon_execute_refund_request(1,1,1,3,'refund-execute-no-inspection');exception when others then blocked:=sqlerrm like '%逐项验收%';end;
 if not blocked or (select status from public.salon_refund_requests where id=3)<>'approved' or (select quantity from public.salon_inventory_balances where catalog_item_id=2)<>8 or exists(select 1 from public.salon_payments where refund_request_id=3) then raise exception 'product refund executed without inspection or leaked writes';end if;
 first:=public.salon_inspect_refund_product_line(1,1,1,3,2,'stockinspect-request-0001',2,0,1,'opened','拆封后确认可再售');retry:=public.salon_inspect_refund_product_line(1,1,1,3,2,'stockinspect-request-0001',2,0,1,'opened','拆封后确认可再售');
 if first<>retry or first->>'acceptedQuantity'<>'1.000' then raise exception 'inspection idempotency failed';end if;
 blocked:=false;begin perform public.salon_inspect_refund_product_line(1,1,1,3,2,'stockinspect-stale-request01',2,0,1.5,'opened','旧版本');exception when others then blocked:=sqlerrm like '%已变化%';end;if not blocked then raise exception 'stale inspection revision accepted';end if;
 perform public.salon_inspect_refund_product_line(1,1,1,3,2,'stockinspect-request-0002',2,1,1.5,'opened','复核后修正验收数量');
 blocked:=false;begin perform public.salon_execute_refund_request(1,1,1,3,'refund-execute-no-channel-01');exception when others then blocked:=sqlerrm like '%尚未复核通过%';end;
 if not blocked or (select status from public.salon_refund_requests where id=3)<>'approved' or (select refunded_total from public.salon_orders where id=1)<>100 or (select quantity from public.salon_inventory_balances where catalog_item_id=2)<>8 then raise exception 'unverified external refund created side effects';end if;
 perform public.salon_record_refund_channel_receipt(1,1,1,3,2,'channel-receipt-report-001',0,'report','provider-ref-123','离线测试：渠道后台已退款');
 blocked:=false;begin perform public.salon_execute_refund_request(1,1,1,3,'refund-execute-pending-001');exception when others then blocked:=sqlerrm like '%尚未复核通过%';end;if not blocked then raise exception 'reported receipt incorrectly treated as verified';end if;
 blocked:=false;begin perform public.salon_record_refund_channel_receipt(1,1,1,3,2,'channel-receipt-selfcheck-01',1,'verify','provider-ref-123','同一人不能复核');exception when others then blocked:=sqlerrm like '%其他员工%';end;if not blocked then raise exception 'maker-checker separation failed';end if;
 perform public.salon_record_refund_channel_receipt(3,1,1,3,2,'channel-receipt-verify-001',1,'verify','provider-ref-123','离线测试：复核登记凭证');
 perform public.salon_execute_refund_request(1,1,1,3,'refund-execute-0003');
 if (select status from public.salon_orders where id=1)<>'reversed' or (select refunded_total from public.salon_orders where id=1)<>200 or (select status from public.salon_payments where id=2)<>'reversed' or (select quantity from public.salon_inventory_balances where catalog_item_id=2)<>9.5 or (select count(*) from public.salon_inventory_ledger where reversal_of_id=1 and refund_request_id=3 and quantity_delta=1.5)<>1 or (select count(*) from public.salon_refund_stock_inspections where refund_request_id=3 and order_line_id=2)<>2 then raise exception 'final refund did not use latest accepted quantity';end if;
