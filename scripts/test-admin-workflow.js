...
assert(admin.includes("row.task_status === 'exempt'") && admin.includes("row.task_status === 'completed'"), 'Meiguanjia records are not classified against hair forms and task decisions');
...
assert(admin.includes('adminCurrentMonthStart') && admin.includes('正在核对本月烫染护记录'), 'reconciliation is not limited to the current month');
assert(admin.includes('adminMgjServiceTypes') && admin.includes("type === 'perm' || type === 'dye' || type === 'care'"), 'reconciliation does not classify service items as perm dye or care');
assert(admin.includes('totalAmount') && admin.includes('typeCounts'), 'reconciliation does not summarize project types and amounts');
assert(admin.includes('adminTransferMgjTask') && admin.includes('adminExemptMgjTask'), 'makeup tasks cannot be transferred or exempted with a reason');
assert(app.includes('renderMgjMakeupTasks') && app.includes('openMgjMakeupTask'), 'employee app has no assigned makeup-task flow');
assert(app.includes('mgjMakeupSourceId') && app.includes("task_status:'completed'"), 'makeup form completion is not linked back to its Meiguanjia task');
assert(admin.includes('未开单 · 本月美管加烫染护对账') && admin.includes('不包含回访任务'), 'missing-order queue is not separated from follow-up work');
