...
result=await handler(request({operation:'operating_report',dateFrom:'2026-09-01',dateTo:'2026-09-30',storeId:999}));assert.equal(result.status,200);assert.equal(calls.at(-1).rpc,'salon_get_operating_report');assert.equal(calls.at(-1).args.p_store_id,9);
