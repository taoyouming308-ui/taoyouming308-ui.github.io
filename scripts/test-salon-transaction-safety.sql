...
insert into public.salon_role_permissions(role_id,resource,action) values(1,'orders','checkout'),(1,'orders','refund'),(1,'inventory','write');
...
select public.salon_refund_order(1,1,1,1,'refund-T-1','顾客取消');
select public.salon_refund_order(1,1,1,1,'refund-T-1','网络重试');
reset role;

do $$ begin
  if (select cash_balance from public.salon_member_accounts where id=1)<>500 then raise exception 'member balance was not restored exactly once'; end if;
  if (select count(*) from public.salon_payments where order_id=1)<>4 then raise exception 'payment reversal count mismatch'; end if;
  if (select count(*) from public.salon_account_ledger where order_id=1)<>2 then raise exception 'member reversal ledger count mismatch'; end if;
  if (select count(*) from public.salon_operation_requests)<>4 then raise exception 'idempotency registry mismatch'; end if;
  if (select quantity from public.salon_inventory_balances where store_id=1 and catalog_item_id=2)<>10 then raise exception 'sold inventory was not returned exactly once'; end if;
  if (select count(*) from public.salon_inventory_ledger)<>3 then raise exception 'inventory reversal count mismatch'; end if;
  if (select status from public.salon_orders where id=1)<>'reversed' then raise exception 'order was not reversed'; end if;
  ...
  if has_function_privilege('authenticated','public.salon_refund_order(bigint,bigint,bigint,bigint,text,text)','EXECUTE') then raise exception 'authenticated can execute refund'; end if;
end $$;
...
do $$ begin
  ...
  begin
    perform public.salon_move_inventory(1,1,1,2,'oversell-P1','sale',11,null,'超卖尝试');
