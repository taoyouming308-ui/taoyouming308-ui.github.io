-- Preserve every explicitly reviewed value, including an intentional blank.
-- Forward-only repair: existing financial rows/candidates are not modified.
alter table public.zysyr_daily_sheet_cell_changes
  add column if not exists value_reviewed boolean not null default false;
