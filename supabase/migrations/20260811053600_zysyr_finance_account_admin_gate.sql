-- ZYSYR V2 Gate C3: one narrowly-scoped administrator capability and an
-- atomic database completion function for direct Supabase Auth finance users.
...
begin
  ...
  select ua.id into strict v_admin_account_id
  from public.zysyr_user_accounts ua
  where ua.company_id = v_company_id
    and lower(btrim(ua.login_name)) = 'admin'
    and ua.status = 'active'
    and exists (
      select 1
      from public.zysyr_user_role_grants g
      join public.zysyr_roles r on r.id = g.role_id
      where g.company_id = ua.company_id
        and g.user_account_id = ua.id
        ...
        and r.code = 'shareholder'
        and r.status = 'active'
    );

  insert into public.zysyr_capabilities (code, name, risk_level)
  values ('finance_account.create', '创建财务账号', 'high')
  ...
  if not exists (
    ...
    where company_id = v_company_id
      and entity_type = 'user_capability_grant'
      and entity_id = v_capability_id
      and action = 'finance_account_creator_enabled'
  ) then
    insert into public.zysyr_audit_events (
      ...
      v_capability_id,
      'finance_account_creator_enabled',
      jsonb_build_object(
        'login_name', 'admin',
        'capability_code', 'finance_account.create',
        'scope_type', 'company'
      ),
      'User approved a secure administrator entry for creating finance accounts.',
      'normal'
    );
  end if;
end
...
create or replace function public.zysyr_admin_complete_finance_account(
...
begin
  ...
  if p_login_name is null
    ...
    raise exception 'invalid finance login name';
  end if;

  if p_display_name is null
     or btrim(p_display_name) = ''
     or char_length(btrim(p_display_name)) > 80 then
    raise exception 'invalid finance display name';
  end if;
  ...
  if (p_scope_type = 'company' and p_store_id is not null)
    ...
    raise exception 'invalid finance scope';
  end if;
  ...
  if not found or not exists (
    ...
    raise exception 'finance account creation is not authorized';
  end if;

  select id into strict v_finance_role_id
  from public.zysyr_roles
  where code = 'finance' and status = 'active';
  ...
  if p_scope_type = 'store' then
    ...
    where company_id = v_actor.company_id
      ...
      and status = 'active';
    if not found then
      raise exception 'finance store scope is not active';
    end if;
  end if;
  ...
  if not found
     ...
     or coalesce(v_auth.raw_app_meta_data ->> 'zysyr_role', '') <> 'finance' then
    raise exception 'Supabase Auth finance identity does not match the request';
  end if;
  ...
  if exists (
    ...
    raise exception 'finance login name or identity already exists';
  end if;
  ...
  insert into public.zysyr_audit_events (
    ...
    'Administrator created a finance account from the secure operations dashboard entry.',
    'personal'
  );
...
revoke execute on function public.zysyr_admin_complete_finance_account(
  uuid, uuid, uuid, text, text, text, uuid, uuid
) from public, anon, authenticated, service_role;

grant execute on function public.zysyr_admin_complete_finance_account(
  uuid, uuid, uuid, text, text, text, uuid, uuid
) to service_role;

comment on function public.zysyr_admin_complete_finance_account(
  uuid, uuid, uuid, text, text, text, uuid, uuid
) is 'Completes one administrator-approved finance Auth identity, role scope, and immutable audit event in one transaction. Passwords never enter Postgres.';
