-- Switch daily-report image candidates from Kimi to OpenAI while preserving
-- all historical source labels and the finance-only review gate.
...
alter table public.zysyr_daily_sheet_cells
  add constraint zysyr_daily_sheet_cells_source_method_check check (
    source_method in (
      'openai_vision','openai_vision_candidate','kimi_vision',
      'kimi_vision_candidate','paddle_ocr','blank_template'
    )
  );
...
begin
  ...
  if jsonb_typeof(p_candidates) <> 'array'
    or jsonb_array_length(p_candidates) not between 1 and 1000
  ...
  where d.company_id=p_company_id and d.store_id=p_store_id and d.id=p_draft_id
  for update;
  if not found then
  ...
  if not found then
    raise exception using errcode='P0002',message='APPROVED_DAILY_VOUCHER_REQUIRED';
  end if;
  if not (
    v_draft.source_voucher_id=p_voucher_id or exists(
      select 1 from public.zysyr_daily_sheet_attachments a
      where a.company_id=p_company_id and a.store_id=p_store_id
        and a.draft_id=p_draft_id and a.voucher_id=p_voucher_id
        and a.attachment_kind='original_report'
    )
  ) then
    raise exception using errcode='42501',message='DAILY_VOUCHER_NOT_LINKED';
  end if;
  ...
  for v_item in select value from jsonb_array_elements(p_candidates) loop
    ...
    where c.company_id=p_company_id and c.store_id=p_store_id
      and c.draft_id=p_draft_id and c.id=(v_item->>'id')::uuid
    for update;
    if not found then
      raise exception using errcode='P0002',message='DAILY_SHEET_CELL_NOT_FOUND';
    end if;
    ...
    if v_cell.manual_override then
      ...
      set ocr_numeric=v_value,ocr_text=v_value::text,confidence=v_confidence,
        source_method='openai_vision_candidate',bbox=null,
  ...
  set edit_revision=v_revision,validation_result=v_validation,
    ocr_provider='openai',ocr_model=btrim(p_model),
    updated_by_user_id=p_actor_user_id,updated_at=now()
  where id=p_draft_id;

  insert into public.zysyr_audit_events(
    company_id,store_id,actor_type,actor_user_id,channel,entity_type,entity_id,
    action,before_json,after_json,reason,sensitivity
  ) values (
    p_company_id,p_store_id,'user',p_actor_user_id,'api','daily_sheet_draft',
    p_draft_id,'daily_recognition_candidates_saved',
    jsonb_build_object('revision',v_draft.edit_revision),
    jsonb_build_object(
      'revision',v_revision,'voucher_id',p_voucher_id,'provider','openai',
      'model',btrim(p_model),'candidate_count',jsonb_array_length(p_candidates),
      'saved_cells',v_saved,'manual_cells_preserved',v_manual_skipped,
      'validation',v_validation
    ),
    'OpenAI原图识别候选已保存，等待财务逐格核对并最终确认','financial'
  );
  return jsonb_build_object(
    'draft_id',p_draft_id,'revision',v_revision,'saved_cells',v_saved,
    'manual_cells_preserved',v_manual_skipped,'validation',v_validation,
    'provider','openai'
  );
end $$;

revoke all on function public.zysyr_apply_daily_sheet_recognition_candidates(
  uuid,uuid,uuid,uuid,uuid,integer,jsonb,text
) from public,anon,authenticated;
grant execute on function public.zysyr_apply_daily_sheet_recognition_candidates(
  uuid,uuid,uuid,uuid,uuid,integer,jsonb,text
) to service_role;
