-- Fix company-scoped finance account creation.
--
-- In PL/pgSQL an unassigned RECORD cannot be dereferenced, even when the
-- dereference appears in the non-selected branch of a CASE expression.  The
-- original function only assigned v_store for store-scoped accounts, so the
-- company-scoped path rolled back while building the response JSON.
...
declare
  ...
  v_store record;
begin
  -- Give the record a known shape for the company-scoped return path.
  select null::uuid as id, null::text as name into v_store;
