-- Explicit row-name review shares the atomic cell save and immutable audit.
-- No existing amounts, names, audit rows or financial status are changed.
alter table public.zysyr_daily_sheet_cell_changes
  add column if not exists row_label_reviewed boolean not null default false;
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_cells) loop
    ...
    if v_numeric_changed or v_text_changed or v_label_changed or v_label_reviewed then
      insert into public.zysyr_daily_sheet_cell_changes(company_id, store_id, draft_id, cell_id,
        ...
        changed_by_user_id, reason, row_label_reviewed)
      values(p_company_id, p_store_id, p_draft_id, v_cell.id, v_revision,
        ...
        p_actor_user_id, btrim(p_reason), v_label_reviewed);
